	...
	<div id="content">
		...
		<div>
			<c:if test="${paging.startPage>paging.blockSize}">
				[ <a href="${conPath }/book.do?method=view&pageNum=${paging.startPage-1 }&schItem=${param.schItem }&schWord=${param.schWrod}">이전</a> ]
			</c:if>	
			<c:forEach var="i" begin="${paging.startPage}" end="${paging.endPage }">
				...
				<c:if test="${paging.currentPage != i }">
					[ <a href="${conPath }/book.do?method=view&pageNum=${i }&schItem=${param.schItem }&schWord=${param.schWord}">${i }</a> ]
				</c:if>
			</c:forEach>
			<c:if test="${paging.endPage<paging.pageCnt }">
				[ <a href="${conPath }/book.do?method=view&pageNum=${paging.endPage+1 }&schItem=${param.schItem }&schWord=${param.schWord}">다음</a> ]
			</c:if>
		</div>
	</div>
