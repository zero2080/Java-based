...
<body>
	...
	<table>
		...
		<c:forEach var="emp" items="${joinlist }">
	...
	<c:if test="${paging.startPage>paging.blockSize }">
		[ <a href="${conPath }/joinlist.do?pageNum=${paging.startPage-1}">이전</a> ]
	</c:if>
	<c:forEach var="i" begin="${paging.startPage }"
		...
		<c:if test="${paging.currentPage!=i }">
			[ <a href="${conPath }/joinlist.do?pageNum=${i}"> ${i }</a> ]
		</c:if>
	</c:forEach>
	<c:if test="${paging.endPage<paging.pageCnt }">
		[ <a href="${conPath }/joinlist.do?pageNum=${paging.endPage+1}">다음</a> ]
	</c:if>
</body>
</html>
